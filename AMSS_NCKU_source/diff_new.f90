...
!-----------------------------------------------------------------------------
!
! Fused first derivatives for the six conformal metric components.
!
  subroutine fderivs_metric6(ex,dxx,gxy,gxz,dyy,gyz,dzz, &
       gxxx,gxxy,gxxz,gxyx,gxyy,gxyz,gxzx,gxzy,gxzz, &
       gyyx,gyyy,gyyz,gyzx,gyzy,gyzz,gzzx,gzzy,gzzz, &
       X,Y,Z,symmetry,onoff)
  implicit none
  ...
  real*8 :: dX,dY,dZ
  real*8,dimension(-1:ex(1),-1:ex(2),-1:ex(3)) :: fhxx,fhxy,fhxz
  real*8,dimension(-1:ex(1),-1:ex(2),-1:ex(3)) :: fhyy,fhyz,fhzz
  real*8, dimension(3) :: SoA
  integer :: imin,jmin,kmin,imax,jmax,kmax,i,j,k
  real*8 :: d12dx,d12dy,d12dz,d2dx,d2dy,d2dz
  ...
  SoA = (/ ONE, ONE, ONE /)
  call symmetry_bd(2,ex,dxx,fhxx,SoA)
  SoA = (/ -ONE, -ONE, ONE /)
  call symmetry_bd(2,ex,gxy,fhxy,SoA)
  SoA = (/ -ONE, ONE, -ONE /)
  call symmetry_bd(2,ex,gxz,fhxz,SoA)
  SoA = (/ ONE, ONE, ONE /)
  call symmetry_bd(2,ex,dyy,fhyy,SoA)
  SoA = (/ ONE, -ONE, -ONE /)
  call symmetry_bd(2,ex,gyz,fhyz,SoA)
  SoA = (/ ONE, ONE, ONE /)
  call symmetry_bd(2,ex,dzz,fhzz,SoA)
  ...
  do j=3,ex(2)-2
  do i=3,ex(1)-2
      gxxx(i,j,k)=d12dx*(fhxx(i-2,j,k)-EIT*fhxx(i-1,j,k) &
                           +EIT*fhxx(i+1,j,k)-fhxx(i+2,j,k))
      gxxy(i,j,k)=d12dy*(fhxx(i,j-2,k)-EIT*fhxx(i,j-1,k) &
                           +EIT*fhxx(i,j+1,k)-fhxx(i,j+2,k))
      gxxz(i,j,k)=d12dz*(fhxx(i,j,k-2)-EIT*fhxx(i,j,k-1) &
                           +EIT*fhxx(i,j,k+1)-fhxx(i,j,k+2))
      gxyx(i,j,k)=d12dx*(fhxy(i-2,j,k)-EIT*fhxy(i-1,j,k) &
                           +EIT*fhxy(i+1,j,k)-fhxy(i+2,j,k))
      gxyy(i,j,k)=d12dy*(fhxy(i,j-2,k)-EIT*fhxy(i,j-1,k) &
                           +EIT*fhxy(i,j+1,k)-fhxy(i,j+2,k))
      gxyz(i,j,k)=d12dz*(fhxy(i,j,k-2)-EIT*fhxy(i,j,k-1) &
                           +EIT*fhxy(i,j,k+1)-fhxy(i,j,k+2))
      gxzx(i,j,k)=d12dx*(fhxz(i-2,j,k)-EIT*fhxz(i-1,j,k) &
                           +EIT*fhxz(i+1,j,k)-fhxz(i+2,j,k))
      gxzy(i,j,k)=d12dy*(fhxz(i,j-2,k)-EIT*fhxz(i,j-1,k) &
                           +EIT*fhxz(i,j+1,k)-fhxz(i,j+2,k))
      gxzz(i,j,k)=d12dz*(fhxz(i,j,k-2)-EIT*fhxz(i,j,k-1) &
                           +EIT*fhxz(i,j,k+1)-fhxz(i,j,k+2))
      gyyx(i,j,k)=d12dx*(fhyy(i-2,j,k)-EIT*fhyy(i-1,j,k) &
                           +EIT*fhyy(i+1,j,k)-fhyy(i+2,j,k))
      gyyy(i,j,k)=d12dy*(fhyy(i,j-2,k)-EIT*fhyy(i,j-1,k) &
                           +EIT*fhyy(i,j+1,k)-fhyy(i,j+2,k))
      gyyz(i,j,k)=d12dz*(fhyy(i,j,k-2)-EIT*fhyy(i,j,k-1) &
                           +EIT*fhyy(i,j,k+1)-fhyy(i,j,k+2))
      gyzx(i,j,k)=d12dx*(fhyz(i-2,j,k)-EIT*fhyz(i-1,j,k) &
                           +EIT*fhyz(i+1,j,k)-fhyz(i+2,j,k))
      gyzy(i,j,k)=d12dy*(fhyz(i,j-2,k)-EIT*fhyz(i,j-1,k) &
                           +EIT*fhyz(i,j+1,k)-fhyz(i,j+2,k))
      gyzz(i,j,k)=d12dz*(fhyz(i,j,k-2)-EIT*fhyz(i,j,k-1) &
                           +EIT*fhyz(i,j,k+1)-fhyz(i,j,k+2))
      gzzx(i,j,k)=d12dx*(fhzz(i-2,j,k)-EIT*fhzz(i-1,j,k) &
                           +EIT*fhzz(i+1,j,k)-fhzz(i+2,j,k))
      gzzy(i,j,k)=d12dy*(fhzz(i,j-2,k)-EIT*fhzz(i,j-1,k) &
                           +EIT*fhzz(i,j+1,k)-fhzz(i,j+2,k))
      gzzz(i,j,k)=d12dz*(fhzz(i,j,k-2)-EIT*fhzz(i,j,k-1) &
                           +EIT*fhzz(i,j,k+1)-fhzz(i,j,k+2))
  enddo
  enddo
  enddo
  ...
  do i=1,ex(1)-1
    ...
    elseif(i+1 <= imax .and. i-1 >= imin .and. &
           j+1 <= jmax .and. j-1 >= jmin .and. &
           k+1 <= kmax .and. k-1 >= kmin) then
      gxxx(i,j,k)=d2dx*(-fhxx(i-1,j,k)+fhxx(i+1,j,k))
      gxxy(i,j,k)=d2dy*(-fhxx(i,j-1,k)+fhxx(i,j+1,k))
      gxxz(i,j,k)=d2dz*(-fhxx(i,j,k-1)+fhxx(i,j,k+1))
      gxyx(i,j,k)=d2dx*(-fhxy(i-1,j,k)+fhxy(i+1,j,k))
      gxyy(i,j,k)=d2dy*(-fhxy(i,j-1,k)+fhxy(i,j+1,k))
      gxyz(i,j,k)=d2dz*(-fhxy(i,j,k-1)+fhxy(i,j,k+1))
      gxzx(i,j,k)=d2dx*(-fhxz(i-1,j,k)+fhxz(i+1,j,k))
      gxzy(i,j,k)=d2dy*(-fhxz(i,j-1,k)+fhxz(i,j+1,k))
      gxzz(i,j,k)=d2dz*(-fhxz(i,j,k-1)+fhxz(i,j,k+1))
      gyyx(i,j,k)=d2dx*(-fhyy(i-1,j,k)+fhyy(i+1,j,k))
      gyyy(i,j,k)=d2dy*(-fhyy(i,j-1,k)+fhyy(i,j+1,k))
      gyyz(i,j,k)=d2dz*(-fhyy(i,j,k-1)+fhyy(i,j,k+1))
      gyzx(i,j,k)=d2dx*(-fhyz(i-1,j,k)+fhyz(i+1,j,k))
      gyzy(i,j,k)=d2dy*(-fhyz(i,j-1,k)+fhyz(i,j+1,k))
      gyzz(i,j,k)=d2dz*(-fhyz(i,j,k-1)+fhyz(i,j,k+1))
      gzzx(i,j,k)=d2dx*(-fhzz(i-1,j,k)+fhzz(i+1,j,k))
      gzzy(i,j,k)=d2dy*(-fhzz(i,j-1,k)+fhzz(i,j+1,k))
      gzzz(i,j,k)=d2dz*(-fhzz(i,j,k-1)+fhzz(i,j,k+1))
    endif
  enddo
  enddo
  enddo
  ...
  end subroutine fderivs_metric6
!-----------------------------------------------------------------------------
!
