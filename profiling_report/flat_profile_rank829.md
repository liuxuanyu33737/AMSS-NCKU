# AMSS-NCKU gprof 函数性能分析

- 分析工具：GNU gprof
- 分析对象：ABE CPU 程序
- MPI进程数：8
- 当前数据来源：一个 MPI 进程 `gmon.out.829`
- 统计到的函数数量：131
...
1. `占比(%)`表示函数自身执行时间占该进程采样总时间的比例。
2. 当前结果来自一个MPI进程，能够用于定位主要计算热点。
3. `compute_rhs_bssn_`是当前最主要的热点函数。
4. 后续优化前后应保持相同输入、步数、MPI进程数和网格设置。
